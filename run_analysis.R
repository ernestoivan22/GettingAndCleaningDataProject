...
if(!file.exists("UCI HAR Dataset")){
  ...
  unzip("data/uci_har_dataset.zip", exdir = ".") 
}
...
# create a table that contains the data we need related to the training
train_table <- cbind(raw_subject_train, raw_y_train, raw_x_train[,features_index])

# create a table that contains the data we need related to the testing
test_table <- cbind(raw_subject_test, raw_y_test, raw_x_test[,features_index])

# Set header names to both tables
...
# Change the id of the activities with their respective label in both tables
